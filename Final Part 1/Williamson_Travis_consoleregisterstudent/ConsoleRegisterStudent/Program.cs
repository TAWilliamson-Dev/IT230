...
namespace ConsoleRegisterStudent
{
    class Program
    {
        ...
        void run()
        {
            ...
            System.Console.WriteLine("Travis Williamson");
        ...
        {
            if (choice < 1 || choice > 7) //Changed comparison from choice > 70 to choice > 7
            {
                return -1;
            }
            else if (choice == firstChoice || choice == secondChoice || choice == thirdChoice) //Changed from a AND b AND c to a OR b OR c
            {
                return -2;
            }
            else if (totalCredit >= 9) //Changed to >= from >
            {
                return -3;
            }
            return 0; //default return value changed from -4 to 0
        }
